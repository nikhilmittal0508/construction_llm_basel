    ...
    try:
        ...
        if len(file_list) > 0:
            with ProcessPoolExecutor(max_workers=2) as executor:
                futures = [executor.submit(ingest_file_process, folder_path,filename,collection_name,model_type) for i, filename in tqdm.tqdm(enumerate(file_list))]
...
def get_answer():
    ...
    if model_type == "openai":
        ...
        # llm = ChatOpenAI(openai_api_key=config["open_api_key"], model="gpt-4o-mini", max_tokens=2000)
