...
def file_ingestion_qdrant():
    ...
    if uploaded_file.filename.lower().endswith((".txt", ".pdf", ".doc", ".xlsx", ".pptx", ".csv", ".html", ".json")):
...
def ingest_folder():
    try:
        ...
        destination_with_selected = os.path.join(destination_folder, os.path.basename(selected_folder))

        print("destination_with_selected: ",destination_with_selected)

        # Copy the entire folder and its contents to the destination
        # Check if the destination folder exists
        if os.path.exists(destination_with_selected):
            print(f"Destination folder '{os.path.basename(selected_folder)}' already exists.")

            # You can choose to overwrite the existing folder or take other actions
            # For example, you might want to prompt the user for confirmation

#             user_response = input("Do you want to overwrite the existing folder? (yes/no): ").lower()

#             if user_response == 'yes':
#                 # If the user confirms, copy the entire folder and its contents to the destination
#                 shutil.copytree(selected_folder, destination_folder)
#                 print(f"Folder '{selected_folder}' has been copied to '{destination_folder}'.")
#             else:
#                 print("Operation aborted.")
        else:
            # If the destination folder doesn't exist, copy the entire folder and its contents to the destination
            shutil.copytree(selected_folder, destination_with_selected)
            print(f"Folder '{os.path.basename(selected_folder)}' has been copied to '{destination_folder}'.")
            

        folder_path = destination_with_selected
        print(folder_path)
        file_list = [filename for filename in os.listdir(folder_path) if filename.lower().endswith((".txt", ".pdf", ".doc", ".xlsx", ".pptx", ".csv", ".html", ".json"))]
...
def get_answer():
    ...
    retriever = qdrant_db.as_retriever(search_type="mmr", search_kwargs={"k": no_of_source})
